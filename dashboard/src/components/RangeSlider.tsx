...
export const RangeSlider=({items, onItemChange}: RangeSliderProps)=>{
  ...
  const handleMax= useCallback((e: { target: { value: string; }; }) =>{
    let val = Math.min(maxIndex, parseInt(e.target.value))
   ...
   return (
    <div className="flex flex-col gap-2 p-4 text-sm text-gray-700">
      <div className="flex items-center gap-3">
        ...
        <input type="range" min="0" max={maxIndex} step="1" value={minValue} onChange={handleMin} className="flex-1 accent-gray-500" />
        <span className="w-20 text-right">{items[minValue]}</span>
      </div>
      <div className="flex items-center gap-3">
        <span className="w-24 text-gray-400">End Date</span>
        <input type="range" min="0" max={maxIndex} step="1" value={maxValue} onChange={handleMax} className="flex-1 accent-gray-500" />
